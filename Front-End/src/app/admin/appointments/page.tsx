...
export default function AppointmentsAdmin() {
  ...
  const {
    ...
    error: listError,
    refetch: refetchList,
  } = useSecureApi<ListResponse<Appointment>>(listUrl)
  ...
  // Parcels (protected) – this endpoint returns an array
  const { data: parcelsData } = useSecureApi<ParcelDto[]>('/api/parcels/all')
  const parcels = Array.isArray(parcelsData) ? parcelsData : []

  // Mutations
  const { mutate, loading: mutLoading } = useSecureMutation()

  // Validation (uses secureApiRequest for uniqueness check)
  const validateForm = async () => {
    ...
    // Unicité email (si création) – recherche d'un RDV PENDING existant
    if (form.contactEmail && !form.id) {
      const { data, error } = await secureApiRequest<ListResponse<Appointment> | Appointment[]>(
        `/api/appointments?search=${encodeURIComponent(form.contactEmail)}`
      )
      if (!error && data) {
        const list = Array.isArray((data as ListResponse<Appointment>)?.items)
          ? (data as ListResponse<Appointment>).items!
          : (Array.isArray(data) ? (data as Appointment[]) : [])
        if (list.some(apt => apt.contactEmail === form.contactEmail && apt.status === 'PENDING')) {
  ...
  const handleStatus = (value: string) => setForm({ ...form, status: value })
  const handleParcel = (value: string) => setForm({ ...form, parcelId: value })
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (form.id) {
        await mutate(`/api/appointments/${form.id}`, body, { method: 'PUT' })
      } else {
        await mutate('/api/appointments', body, { method: 'POST' })
      }
  ...
  async function del(id: string) {
    await mutate(`/api/appointments/${id}`, undefined, { method: 'DELETE' })
    await refetchList()
  }
                  ...
                  <td className="p-2 align-top">
                    <div className="max-w-xs">
                      <p
                        ...
                        title={a.projectDescription || 'Aucune description'}
                      >
                    ...
                    <span
                      className={`px-2 py-1 rounded text-xs ${
                        a.urgency === 'urgent'
                          ...
                          : a.urgency === 'moyen-terme'
                          ? 'bg-yellow-100 text-yellow-800'
                          : a.urgency === 'long-terme'
        ...
        <DialogContent>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={form.activityType || undefined}
                onValueChange={(value) => setForm({ ...form, activityType: value })}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Select
                value={form.investmentBudget || undefined}
                onValueChange={(value) => setForm({ ...form, investmentBudget: value })}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Select
                value={form.preferredTime || undefined}
                onValueChange={(value) => setForm({ ...form, preferredTime: value })}
              >
                <SelectTrigger>
